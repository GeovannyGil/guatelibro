...
function cargar(id) {
    ...
    $("#cargar" + id).parents("tr").find("td").each(function () {
        ...
        if (conteo == 1) {
            document.form_library_userA.selectMemberU.value = $(this).html();
        }
        if (conteo == 2) {
            document.form_library_userA.selectProductU.value = $(this).html();
        }
...
function actualizar_library_user() {
    ...
    $.ajax({
        type: "POST",
        url: "http://localhost/guatelibro/Modificar/library_user",
        data: formData,
        cache: false,
        contentType: false,
        processData: false
